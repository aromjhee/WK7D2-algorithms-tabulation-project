...
function stepper(nums, memo = {}) {
    // let key = String(nums); // string ver of nums array is too big
    // console.log(key); // notice start of the arrays or length, are diff
...
function maxNonAdjacentSum(nums) {
    ...
    Math.max(firstEle + maxNonAdjacentSum(nums.slice(2)), maxNonAdjacentSum(nums.slice(1)));
}
...
function minChange(coins, amount) {
    ...
    coins.forEach( val => {
        for (let amnt = 0; amnt < table.length; amnt++) {
            for (let qty = 0; qty * val <= amnt; qty++) {
                let remainder = amnt - qty * val
                let attempt = table[remainder] + qty
                if (attempt < table[amnt]) table[amnt] = attept;
            }
        }
    })
    return table[table.length - 1];
